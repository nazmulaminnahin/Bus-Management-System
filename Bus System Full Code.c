...
char ch[10][130]={"Hanif","Shyamoli","Green Line","Ena","Asia Line"};
char name[32][100]={'\0'};
char number[32][2]={'\0'};
int num1[32]={0};
int trno;
...
seat1=atoi(str1);//converting the string into number
if(seat1 <= 0)
{
...
itoa(seat1, str1, 10);
//reading the seats from the user.
if(trno == 1)
{
 f1 = fopen("tr1.txt","w");
 fputs(str1,f1);
...
int number;
   ...
   a = fopen(tempstr1,"a");//opening the file to write the name in the file
   b = fopen(tempstr2,"a");//opening the file for writing the number in the file
for(i=0; i<booking; i++)//entering the person name and seat number in the file
{
...
strcat(numstr,".txt");
strcat(tempstr2,numstr);
a = fopen(tempstr2,"a+");//open the file to write the name in the file
   while(!feof(a))
   {
      number[i][j] = fgetc(a);
...
void read_name(int trno)//putting the numeric value in the array
{
...
strcat(numstr,".txt");
strcat(tempstr1,numstr);
b = fopen(tempstr1,"a+");//opening the file to write the name in the file
   while(!feof(b))
   {
      name[i][j] = fgetc(b);
    ...
    do
{
	...
	pword[i]='\0';
	i=0;
		if(strcmp(uname,"nazmul")==0 && strcmp(pword,"12345")==0)
	{
    printf("\n   \xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd\xcd");
	printf("  \n\n   WELCOME USER !!!!");
